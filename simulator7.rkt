...
(define BLOCK2 (make-block (list 10 0) (make-list 5 #f) (list 1 0) (list 1 0)))
...
(define (create-intersections streets)
  (let* ([street-pairs (produto-cartesiano (take streets 10) (drop streets 10))]
         [intersections
...
(define-struct simulator (streets intersections car-buffer
                                    count-iter car-id car-times) #:transparent)
;example:
(define SIM1 (make-simulator (list STR1  STR2) (list INT1) empty 0 0 empty))

(define (init-simulator)
  (let* (
         [streets
          (append (map (λ (i) (create-street i (first DIRECTIONS)))
            ...
            (cond [(empty? blocks) #f]
                  [else
                   (or (and (equal? (block-id (first blocks)) (car-next-block-id car))
                            (false? (first (block-lane (first blocks)))))
...
(define (add-time-cars cars)
  (cond [(empty? cars) empty]
        [else
         (cons (if (false? (first cars))
                   #f
                   (make-car
                    (car-id (first cars))
                    (car-block-id (first cars))
                    (car-dir (first cars))
                    (add1 (car-total-time (first cars)))
                    #f
                    (car-next-block-id (first cars))))
               (add-time-cars (rest cars)))]))

(define (try-flow-blocked-lane v normal-flow?)
  (local [
          (define (try-flow-blocked-lane-aux lane i)
            (cond [(< i 1) lane]
                  [else
                   
                   (try-flow-blocked-lane-aux
                    (if (or normal-flow? (false? (list-ref lane i)))
                        (begin
                          
                          (for ([j (in-range (sub1 i) -1 -1)]
                                #:when (not (false? (list-ref lane j))))                                         
                            (log-move-car (car-id (list-ref lane j)) #f)
                            )
                          (append (list #f) (take lane i) (drop lane (add1 i))))
                        lane)
                    (if (or normal-flow? (false? (list-ref lane i)))
                        0
                        (sub1 i)))]
    ...
    (if (andmap false? lane) lane
        (add-time-cars (try-flow-blocked-lane-aux lane 4)))))
  ...
  (let* ([moving-out (last (block-lane block))]
    ...
    (list (try-flow-blocked-lane (block-lane block) normal-flow?)                
          (cond [not-blocked-intersection?
                 int-entered]
                [else ints])
          (if exiting? (car-total-time moving-out) #f)))))
...
(define (move-blocks blocks ints int-cars)
  (local [
          (define (move-blocks-aux blocks ints blocks-acc exitted-times)
            (cond [(empty? blocks) (list blocks-acc ints exitted-times)]
                  [else
                   (let* ([trying (move-block (first blocks) ints)]
                     ...
                     (move-blocks-aux (rest blocks)
                                      ...
                                      (if (not (false? (third trying))) (cons (third trying) exitted-times) exitted-times)
                                      ))]))
...
(define (try-move-streets streets ints int-cars)
  (local [
          (define (try-move-streets-aux streets ints str-acc exitted-cars)
            (cond [(empty? streets) (list str-acc ints exitted-cars)]
                  [else
                   (let ([moving (try-move-street (first streets) ints int-cars)])
                     (try-move-streets-aux (rest streets)
                                           ...
                                           (if (not (empty? (third moving))) (append (third moving) exitted-cars) exitted-cars)
                                           ))]
                  ))]
...
(define (insert-car-entry-block blocks car-id street)
  (local [
          (define (insert-car-entry-block-aux blocks car-id street blocks-acc car-not-added)
            (cond [(empty? blocks) (list blocks-acc car-not-added)]
                  [else
                   (let* ([found-block? (equal? (block-id (first blocks)) (street-entry-block street))]
                          [block-full? (if found-block?
                                           (andmap (λ (x) (not (false? x))) (block-lane (first blocks)) )
                                           #f)])

                     (insert-car-entry-block-aux (rest blocks) car-id street
                                                 (cons (if (and found-block? (not block-full?))
                                                           (begin
                                                 ...
                                                 (if block-full?
                                                     (begin (log-info (string-append "Car " (number->string car-id) " added to buffer"))
                                                     (list (street-id street) car-id)) car-not-added)))]))             
          ]
...
(define (insert-cars streets car-id)
  (local [
          (define (insert-cars-aux streets car-id streets-acc cars-not-added)
          (cond [(empty? streets) (list streets-acc cars-not-added)]
                [else
                 (let [(trying (insert-car (first streets) car-id))]
                   (insert-cars-aux (rest streets) (add1 car-id)
                                    (cons (first trying) streets-acc)
                                    (if (not (false? (second trying)))
                                        (cons (second trying) cars-not-added)
                                        cars-not-added))
                )]))
        ]
...
(define (insert-car-street streets str-id car-id)
  (local [
          (define (insert-car-street-aux streets str-id car-id streets-acc buffer-acc)
            ...
            (cond [(empty? streets) (list streets-acc buffer-acc)]
                  [(equal? (street-id (first streets)) str-id)
                   (let ([trying (insert-car (first streets) car-id)])
                     (insert-car-street-aux
                      (rest streets) str-id car-id 
                      (cons (first trying) streets-acc)
                      (if (not (false? (second trying)))
                          (cons (second trying) buffer-acc)
                          buffer-acc)))]
                  [else  (insert-car-street-aux
                          (rest streets) str-id car-id
                          (cons (first streets) streets-acc)
                          buffer-acc)]))]
...
(define (insert-cars-street-from-buffer streets buffer)
  (local [
          (define (insert-cars-street-from-buffer-aux streets buffer buffer-acc)            
            (cond [(empty? buffer) (list streets buffer-acc)]
                  [else
                   (let ([trying (insert-car-street streets (first (first buffer)) (second (first buffer)))])
                   (insert-cars-street-from-buffer-aux (first trying)
                                                        (rest buffer)
                                                        (append (second trying) buffer-acc)
                                                        ))]))]
...
(define (tick sim)
  (begin
    ...
    (let* (
           ...
           [try-move-cross (try-move-crossings ticked-ints  (simulator-streets sim)) ]
           [intersections-moved1 (first try-move-cross)]
           [to-move-out-crossing (second try-move-cross)]
         
           [try-move-street (try-move-streets (simulator-streets sim) intersections-moved1 to-move-out-crossing)]
           [streets-moved1 (first try-move-street)]
           [intersections2 (second try-move-street)]

           [exitted-cars-times (if (not (empty? (third try-move-street)))
                                   (third try-move-street) #f)]
         
           [insert-car? (= (remainder (simulator-count-iter sim) CAR-SPAWN-RATE) 0)]
           ;[v-next-id (+ CAR-SPAWN-RATE (simulator-car-id sim))]

           [try-insert-car (if insert-car?
             (insert-cars streets-moved1 (simulator-car-id sim))
             #f)]
           
           [streets-moved2 (if insert-car?
                               (first try-insert-car)
                               streets-moved1)]

           [try-insert-buffer (if (not (empty? (simulator-car-buffer sim)))
                                  (insert-cars-street-from-buffer streets-moved2 (simulator-car-buffer sim))
                                  #f)]
           [streets-moved3 (if (not (false? try-insert-buffer))
                               (first try-insert-buffer)
                               streets-moved2)]
           [new-buffer (if (not (false? try-insert-buffer))
                           (second try-insert-buffer)
                           (simulator-car-buffer sim))]
           )
      ...
      (begin
        (make-simulator
         streets-moved3
         intersections2
         (if (and insert-car? (not (empty? (second try-insert-car))))
             (append (second try-insert-car) new-buffer)
             new-buffer)
         (add1 (simulator-count-iter sim))
         (if insert-car? (+ COUNT-STREETS (simulator-car-id sim)) (simulator-car-id sim))
         (if (not (false? exitted-cars-times))
             (append exitted-cars-times (simulator-car-times sim))
             (simulator-car-times sim))
         )))))

;;; Simulator -> Image
(define (draw sim) empty-image)
(define T-START (current-milliseconds))
(define FINAL (main SIMT1))
(display (- (current-milliseconds) T-START))
(display "\n Media do tempo dos carros: ")
(display (/ (foldl + 0 (simulator-car-times FINAL)) (length (simulator-car-times FINAL))))
